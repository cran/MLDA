...
size<-ncol(mlda.obj$ratio1)

m<-1
for(i in 1:size){

plot(ratio1[,i],ratio2[,i],xlab="log likelihood ratio1",ylab="log likelihood ratio2",main=mlda.data$sample.names[i])
...
points(ratio1[probeID==loci[j],i],ratio2[probeID==loci[j],i],col=1+j,pch=16)

}
if(legend){

legend("topleft",loci,col=c(2:(1+length(loci))),pch=16,cex=0.6)
