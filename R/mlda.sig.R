`mlda.sig` <-
function(score.obj,dataset,loci.ID=NULL,pvalue=0.01,filter=TRUE){
...
sr.obj<-score.obj$sr.obj
			...
			cc <- 1
			while (!is.na(sr.obj$fdr.pos[cc,1]) && cc<500 && sr.obj$fdr.pos[cc,1]>pvalue) {  
			...
			cc <- 1
			while (!is.na(sr.obj$fdr.neg[cc,1]) && cc<500 && sr.obj$fdr.neg[cc,1]>pvalue) {  
			...
			pos.loci.sig<-cbind(loci.ID[up.flag&!flag],sr.obj$score.se[up.flag&!flag],sr.obj$score.re[up.flag&!flag])
			neg.loci.sig<-cbind(loci.ID[down.flag&!flag],sr.obj$score.se[down.flag&!flag],sr.obj$score.re[down.flag&!flag])
			...
			if(neg.count>1&pos.count>1)		
				colnames(pos.sig)<-colnames(neg.sig)<-c("loci ID","sensitive score","resistant score")
			if(neg.count==1)
				names(neg.sig)<-c("loci ID","sensitive score","resistant score")
			if(pos.count==1)
				names(pos.sig)<-c("loci ID","sensitive score","resistant score")
			result<-list(loci.up = pos.sig,loci.down = neg.sig,pos.p = pos.p,neg.p = neg.p)
			result
}
